...
async function checkDatabaseConnection() {
    try {
        ...
        if (isConnected) {
            ...
            dbStatusIndicator.title = 'Database: Connected';
        } else {
            dbStatusIndicator.classList.remove('connected');
        ...
        const dbStatusIndicator = document.getElementById('dbStatusIndicator');
        dbStatusIndicator.classList.remove('connected');
        dbStatusIndicator.title = 'Database: Connection Error';
        return false;
    }
}
...
function showTryAgainMessage() {
    ...
    resultMessage.style.cssText = `
        ...
        white-space: nowrap;
        opacity: 0;
        animation: fadeIn 0.5s forwards, floatUp 2s 0.5s forwards;
    `;

    // Create confetti effect
    createConfetti(coinRect);

    // Create sparkles
    if (Math.random() > 0.5) {
        createSparkles(coinContainer);
    }

    // Clean up after animation
    setTimeout(() => {
        resultMessage.style.animation = 'fadeOut 0.5s forwards';
        setTimeout(() => {
...
function createConfetti(containerRect) {
    const confettiCount = 30;
    const colors = ['#FFD700', '#FF6347', '#00FF7F', '#1E90FF', '#9370DB'];
    
    for (let i = 0; i < confettiCount; i++) {
        const confetti = document.createElement('div');
        confetti.className = 'confetti';
        
        // Set random values as CSS variables
        const randomX = Math.random() * 400 - 200; // Random value between -200 and 200
        const randomRotate = Math.random() * 360; // Random rotation between 0 and 360
...
function createSparkles(container) {
    ...
    for (let i = 0; i < sparkleCount; i++) {
        ...
        // Position randomly around the coin
        ...
        sparkle.style.cssText = `
            ...
            z-index: 1000;
            opacity: 0;
            animation: sparkle-move 1.5s ease-out ${i * 0.1}s forwards;
        `;
